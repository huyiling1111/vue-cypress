...
const store = createStore({
  ...
  getters: {
    ...
    topEvents(state) {
      return state.events.sort((a, b) => new Date(a.date) - new Date(b.date)).filter((event) => !event.cancelled && moment(event.date).isAfter()).slice(0, 3);
    },
    event: (state) => (id) => {
